...
def select_mtcnn_device(use_gpu: bool) -> DetectorDeviceSelection:
    ...
    try:
        ...
        return DetectorDeviceSelection(
            requested="GPU",
            actual="cpu",
            description="要求 GPU，但缺少 torch，已改用 CPU (cpu)",
            used_gpu=False,
        )
    ...
    if not torch.cuda.is_available():
        return DetectorDeviceSelection(
            requested="GPU",
            actual="cpu",
            description="要求 GPU，但未偵測到可用的 CUDA 裝置，已改用 CPU (cpu)",
            used_gpu=False,
        )

    try:
        gpu_name = str(torch.cuda.get_device_name(0) or "cuda:0")
    except Exception:
        gpu_name = "cuda:0"
    return DetectorDeviceSelection(
        requested="GPU",
        actual="cuda:0",
        description=f"要求 GPU，使用 cuda:0 | PyTorch 裝置={gpu_name}",
        used_gpu=True,
    )
...
class LooseFaceDetector:
    def __init__(
        ...
        try:
            ...
            raise SystemExit("缺少 facenet-pytorch / torch，請先安裝對應套件。") from exc
        ...
        self.mtcnn = MTCNN(
            ...
            post_process=False,
            min_face_size=18,
            thresholds=[0.45, 0.55, 0.6],
            factor=0.6,
            device=torch.device(self.active_device),
        )
    ...
    def _mtcnn_hit(self, frame_bgr, angle: int) -> Optional[DetectionHit]:
        ...
        for box_xyxy, confidence, landmark_points in zip(boxes, probabilities, landmarks):
            if box_xyxy is None or landmark_points is None:
                continue
            ...
            keypoints = {
                "left_eye": tuple(float(value) for value in landmark_points[0]),
                "right_eye": tuple(float(value) for value in landmark_points[1]),
                "nose": tuple(float(value) for value in landmark_points[2]),
                "mouth_left": tuple(float(value) for value in landmark_points[3]),
                "mouth_right": tuple(float(value) for value in landmark_points[4]),
            }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--gpu",
        action="store_true",
        help="要求 facenet-pytorch MTCNN 使用 cuda:0；若本機沒有可用 CUDA 裝置，會自動退回 CPU。",
    )
    parser.add_argument(
        "--ignore-overrides",
        action="store_true",
