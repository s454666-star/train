...
def extract_page_info(text: Optional[str]) -> Optional[Dict[str, int]]:
    if not text:
        return None

    s = strip_markdown_stars(text)

    m = re.search(r"第\s*(\d{1,3})\s*/\s*(\d{1,3})\s*[页頁]", s)
    if m:
        cur = int(m.group(1))
        total = int(m.group(2))
        if 1 <= cur <= total <= 999:
    ...
    m2 = re.search(r"(?:page|Page|PAGE)\s*(\d{1,3})\s*/\s*(\d{1,3})", s)
    if m2:
        cur = int(m2.group(1))
    ...
    m3 = re.search(r"\b(\d{1,3})\s*/\s*(\d{1,3})\b", s)
    if m3:
        cur = int(m3.group(1))
...
def _extract_page_button_number(text: str) -> Optional[int]:
    ...
    只把「真正的頁碼按鈕」解析成數字：
    - 允許：✳️1、✅2、 3  這種
    - 排除：获取全部(467)、视频(250) 這種「括號內是總數」的
    - 排除：第2页、2/47 這種描述文字
    """
    t = (text or "").strip()
    if not t:
    ...
    if "/" in t or "第" in t or "页" in t or "頁" in t:
        return None

    m = re.match(r"^\s*(?:✅|☑️|☑|✳️|✳|⭐|🌟|⬜|⬛)?\s*(\d{1,3})\s*$", t)
    if not m:
        m = re.match(r"^\s*(?:[^\dA-Za-z]{0,6})\s*(\d{1,3})\s*$", t)
        if not m:
            return None
...
def _button_marker_score(t: str) -> int:
    s = (t or "").strip()
    score = 0
...
def pick_next_by_keywords(callback_msg: Dict[str, Any], keywords: List[str]) -> Optional[Dict[str, Any]]:
    for b in get_callback_buttons(callback_msg):
        t = (b.get("text") or "").strip()
        for kw in keywords:
            if kw and kw in t:
...
def pick_best_button_by_keywords(callback_msg: Dict[str, Any], keywords: List[str]) -> Optional[Dict[str, Any]]:
...
def _pagination_state_fingerprint(state: Dict[str, Any]) -> str:
    pi = state.get("page_info") or {}
    cur = pi.get("current_page")
    total = pi.get("total_pages")
    highlighted = state.get("highlighted_current")
    numeric_set = state.get("numeric_set") or []
    has_next = state.get("has_next")
    msg_id = state.get("message_id")
    has_buttons = state.get("has_buttons")
    total_items = state.get("total_items")
    is_pagelike = state.get("is_pagination_like")
    preview = (state.get("text_preview") or "")[:120]
    nums_str = ",".join([str(x) for x in numeric_set])
    return f"mid={msg_id}|cur={cur}|total={total}|hl={highlighted}|nums={nums_str}|hasnext={has_next}|btn={has_buttons}|items={total_items}|pagelike={is_pagelike}|pv={preview}"

def get_logs_tail(limit: int = 200) -> List[Dict[str, Any]]:
    return DEBUG_LOGS[-max(int(limit or 200), 1):]

def _pagination_state_changed(a: Dict[str, Any], b: Dict[str, Any]) -> bool:
    if not a or not b:
        return True
    return _pagination_state_fingerprint(a) != _pagination_state_fingerprint(b)
...
def _extract_page_target_number(text: str) -> Optional[int]:
    """
    用於「點指定頁碼」：允許純數字，或兩側帶箭頭/符號（例如 ⬅️1、1➡️）
    但不允許出現英文字母或中文（避免把 Prev/上一頁 之類誤當頁碼）
    """
    s = str(text or "").strip()
    if not s:
        return None
...
def _extract_strict_current_page_candidate(text: str) -> Optional[int]:
    """
    用於「判斷目前頁碼」：只接受純數字或帶高亮標記（例如 ✅2、✳2）
    明確排除任何箭頭（避免把 ⬅️1、1➡️ 當成目前頁碼）
    """
    s = str(text or "").strip()
    if not s:
        return None
...
class RunAllPagesByBotOnlyRequest(BaseModel):
    ...
    # 預設改為不做「獲取全部」
    bootstrap_click_get_all: bool = False
    bootstrap_get_all_keywords: List[str] = ["獲取全部", "获取全部", "Get all", "全部获取", "全部獲取"]
    wait_after_bootstrap_timeout_seconds: int = 25

    # 預設改為不送文字「下一頁」
    text_next_fallback_enabled: bool = False
    text_next_command: str = "下一頁"
    ...
    # 預設改為不做「上一頁/送 1」類型的回到第一頁動作
    normalize_to_first_page_when_no_buttons: bool = False
    normalize_prev_command: str = "上一頁"
    normalize_max_prev_steps: int = 6
