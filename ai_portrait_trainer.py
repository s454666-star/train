        ...
        for attempt in range(1, max_retries + 1):
            ...
            if self.connection and self.connection.is_connected():
                try:
                    ...
                    print(f"插入訓練紀錄錯誤 (嘗試 {attempt}/{max_retries}): {e}")
                    self.connection = None  # 斷開連接以便下次重新連接
                    time.sleep(1)  # 等待1秒後重試
        print(f"無法在 {max_retries} 次嘗試後插入訓練紀錄: {image_path}")
        return False
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if self.connection and self.connection.is_connected():
                try:
                    ...
                    print(f"插入生成圖片紀錄錯誤 (嘗試 {attempt}/{max_retries}): {e}")
                    self.connection = None  # 斷開連接以便下次重新連接
                    time.sleep(1)  # 等待1秒後重試
        print(f"無法在 {max_retries} 次嘗試後插入生成圖片紀錄: {generated_image}")
        return False

# 圖片處理工具
class ImageProcessor:
    def __init__(self, target_size=(256, 256), scale_factor=0.3):
...
class TextToImageModel:
    def __init__(self, model_name="stabilityai/stable-diffusion-3.5-large", torch_dtype=torch.float32):
        ...
        try:
            ...
            # 初始化管道，直接使用 from_pretrained 方法載入
            ...
            print("Stable Diffusion 3 Pipeline 模型載入成功")
        except Exception as e:
            print(f"載入模型時發生錯誤: {e}")
            self.pipe = None
    ...
    def run(self):
        ...
        for idx, path in enumerate(self.image_paths):
            ...
            if img is not None:
                ...
                if not success:
                    self.log.emit(f"無法記錄訓練資料到資料庫，跳過圖片: {path}")
                    print(f"無法記錄訓練資料到資料庫，跳過圖片: {path}")
                batch_counter += 1
